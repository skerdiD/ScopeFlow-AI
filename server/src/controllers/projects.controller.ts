...
export const listProjectController: RequestHandler = async (req, res) => {
  const user = auth(req);
  const rows = await projects.listProjects(user.username);
...
export const createProjectController: RequestHandler = async (req, res) => {
  ...
  const project = await projects.createProject(user.username, input, user.isDemo);
  res.status(201).json(serializeProject(project));
};

export const getProjectController: RequestHandler = async (req, res) => {
  const user = auth(req);
  res.json(serializeProject(await projects.getProject(user.username, projectId(req))));
};
...
export const updateProjectController: RequestHandler = async (req, res) => {
  ...
  res.json(serializeProject(await projects.updateProject(user.username, projectId(req), input)));
};

export const deleteProjectController: RequestHandler = async (req, res) => {
  const user = auth(req);
  await projects.deleteProject(user.username, projectId(req), user.isDemo);
  res.status(204).end();
};
...
export const restoreVersionController: RequestHandler = async (req, res) => {
  ...
  res.json(serializeProject(await projects.restoreVersion(user.username, projectId(req), input.version_id)));
};
...
export const markFinalController: RequestHandler = async (req, res) => {
  ...
  res.json(serializeProject(await projects.markFinal(user.username, projectId(req), input)));
};

export const shareLinkController: RequestHandler = async (req, res) => {
  const user = auth(req);
  const input = parseInput(shareLinkSchema, req.body);
  res.json(serializeProject(await projects.manageShareLink(user.username, projectId(req), input.operation, user.isDemo)));
};
